...
#==========================================
## Function to faciitate sampling of categorical variable
#(Adapted from Ru, B., Alvi, A. S., Nguyen, V., Osborne, M. A. & Roberts, S. J. Bayesian Optimisation over Multiple Continuous and Categorical Inputs.)
#==========================================
import math
import collections
import pickle
...
def compute_prob_dist_and_draw_hts(Wc_list, gamma_list, C_list, batch_size):
    if batch_size > 1:
        ...
        for j in range(len(C_list)): # Loop through the categorical variable
            ...
            gamma = gamma_list[j]
            C = C_list[j]
            # perform some truncation here - HN Comments, get rid of this in the next version. once other issues are fixed.
            maxW = np.max(Wc)
        ...
        return ht_list, probabilityDistribution_list, S0
...
# Drawing choice
def draw(weights):
    choice = uniform(0, sum(weights)) #random.
    #    print(choice)
    choiceIndex = 0
