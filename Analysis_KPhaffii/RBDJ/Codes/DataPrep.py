...
def data_prep(main_file_path, N_round_input, Rounds_bfr_check, cut_off_round, property_name):
    ...
    for nr in range(1, N_round_input + 1):
        ...
        if nr > cut_off_round:
            for i in np.arange(1, cut_off_round + 1):
                if i == 1:
                    data_trim = data[i][:-19]
            ...
            mu_y = np.mean(res_trim,0)
            std_y = np.std(res_trim,0)

            gp_actual, result_pred_ts = predict_internal(data_trim, res_trim, data_param,
                                                         cont_kernel_name, data_ts[nr])

            gp_actual, result_pred_tr = predict_internal(data_trim, res_trim, data_param,
        ...
        if nr != N_round_input:
            if nr > cut_off_round:
                ...
                plt.scatter(result_ts[nr][:-1], Pred[nr], s=60)  # [:-1]
                plt.scatter(res_trim, Train[nr], s=60)
            ...
            plt.scatter(result_ts[nr][-8:], Pred[nr][-8:], s=60)  # [:-1]
            plt.scatter(res_trim, Train[cut_off_round], s=60)
            plt.plot(result[cut_off_round], result[cut_off_round], '--k')
            plt.xlabel('Observed -' + property_name)
            plt.ylabel('Predicted - ' + property_name)
            plt.legend(['Round' + str(nr), 'Round 0-' + str(nr - 1)], loc='upper center',
